...
RSpec.describe Spendings::FormComponent, type: :component do
  let(:date) { Date.today }
  let(:kind) { "shopping" }
  let(:component) { described_class.new(date:, kind:) }
  ...
  it 'renders the form with the correct month and year and kind' do
    expect(page).to have_selector('select[name="kind"]')
    expect(page).to have_select('kind', selected: "Zakupy")
