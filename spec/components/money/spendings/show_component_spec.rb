...
#   let(:spendings_today) { spendings.where(date: date).sum(:amount) }
#   let(:spendings_this_month) { spendings.where(date: date.beginning_of_month..date.end_of_month).sum(:amount) }
#   let(:component) { described_class.new(spendings:, date:) }
#
